...
${skills || '없음'}
  ...
  async analyzeJobPosting(content: string): Promise<JobPostingAnalysisResult> {
    const prompt = `
...
응답 형식 (JSON):
{
  ...
  "companyInfo": "회사가 중시하는 가치나 전반적인 요약 (1~2문장)"
    ...
    try {
      const model = this.ai.getGenerativeModel({ model: 'gemini-3.1-flash-lite-preview' });
      ...
      return JSON.parse(text) as JobPostingAnalysisResult;
    } catch (e) {
...
`;
  ...
  async generateInterviewerResponse(messages: MessageForInterview[], difficulty: 'BASIC' | 'INTERMEDIATE' | 'ADVANCED'): Promise<string> {
...
`;
    ...
    const contents = formattedMessages.length > 0 ? formattedMessages : [{
      role: 'user',
      parts: [{ text: "면접을 시작하겠습니다." }]
    }];
    ...
    try {
      ...
      return response.response.text() || '답변을 생성하지 못했습니다.';
    } catch (e) {
      return '답변을 생성하지 못했습니다.';
    }
  }

  async generateInterviewFeedback(messages: MessageForInterview[]): Promise<InterviewFeedbackResult> {
...
`;
    ...
    try {
      ...
      const text = response.response.text() || '{}';
      return JSON.parse(text) as InterviewFeedbackResult;
    } catch (e) {
      return {
        overallScore: 0,
        categories: [],
